...
// 🔐 Geschützte Download-Route für die SQLite-Datenbank (NUR für Admins mit Token)
const fs = require('fs'); // ggf. schon oben im Code vorhanden

const DOWNLOAD_SECRET = '1q2w3e4r@Sbinpjhh112'; // ⚠️ fester Token – NICHT für Produktion geeignet

app.get('/download-db', (req, res) => {
  // ✅ Nur Admins dürfen auf diese Route zugreifen
  if (!req.session.user || req.session.user.role !== 'admin') {
    return res.status(403).send('Zugriff verweigert – Admin erforderlich.');
  }

  // ✅ Token überprüfen
  if (req.query.token !== DOWNLOAD_SECRET) {
    return res.status(401).send('Ungültiger oder fehlender Token.');
  }

  // 📁 Pfad zur SQLite-Datei
  const filePath = path.join(__dirname, 'db', 'database.sqlite');

  // ❗️Existiert die Datei?
  if (!fs.existsSync(filePath)) {
    return res.status(404).send('Datenbank-Datei nicht gefunden.');
  }

  // ✅ Datei mit Datumsstempel ausliefern
